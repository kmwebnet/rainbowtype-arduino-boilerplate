...
#define ROE(x,msg) { if (((x)<0)) { log_e("LWIP Socket config of " msg " failed."); return -1; }}
    ...
    if (cli_cert != NULL && cli_key != NULL) {
        ...
        if (ret != 0)
	    {
	        log_e("Failed to init ECC608:%d\n", ret);
        }

        keynum = sysinit();

        /* Extract the device certificate and convert to mbedtls cert */
        ret = atca_mbedtls_cert_add(&ssl_client->client_cert, &g_cert_def_2_device);
        if (ret != 0)
        {
            return handle_error(ret);
        }
        /* Parse client private key... */
        ret = atca_mbedtls_pk_init(&ssl_client->client_key,  (uint16_t)keynum);
        if (ret != 0)
        {
            return handle_error(ret);
        }
        ...
        ret = atca_mbedtls_cert_add(&ssl_client->client_cert, &g_cert_def_1_signer);
        if (ret != 0)
        {
            return handle_error(ret);
        }
    ...
    mbedtls_ssl_conf_curves(&ssl_client->ssl_conf, curve);
